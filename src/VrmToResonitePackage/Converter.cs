...
internal static class Converter
{
    public static async Task<int> Run(CliOptions options, string resonitePath)
    {
        ...
        string logPath = Path.Combine(appData, "Logs", $"convert_{DateTime.Now:yyyyMMdd_HHmmss}.log");
        Directory.CreateDirectory(Path.GetDirectoryName(logPath));
        using var logWriter = new StreamWriter(logPath) { AutoFlush = true };
        HookLogging(logWriter);
    ...
    private static void HookLogging(StreamWriter writer)
    {
        object gate = new();
        ...
        UniLog.OnLog += message => WriteLine("", message, toConsole: false);
        UniLog.OnWarning += message => WriteLine("WARN: ", message, toConsole: true);
        UniLog.OnError += message => WriteLine("ERROR: ", message, toConsole: true);
    }
