...
- **FBX モデルの Prefab Variant 対応**: 1 つの全部入り FBX から派生 prefab を作る構造（Kipfel 等）に
  ...
  import 後に欠落した shape を空 frame として元位置へ挿入してから prefab weight を適用する。
...
- **外側の完成形 prefab**: VRCAvatarDescriptor が内側の素体 prefab にだけあり、外側 prefab が素体と
  髪 prefab を合成する場合がある。Descriptor を持つファイルだけで候補一覧を作らず、Descriptor 付き
  prefab を再帰参照する外側 prefab も選択候補に含め、内側 Descriptor の設定を継承する。
...
- Nested prefab property targets are not guaranteed to have a serialized stripped document in the immediate base prefab, so some derived material target fileIDs cannot be followed back to an FBX renderer. For material-family variants, use a strict fallback only when the unresolved override count exactly matches the remaining base-material slot count and the names differ by a one-segment variant marker (`E_Costume -> E_A_Costume`). Eku_Another requires this for Jacket and ten other costume renderers.
